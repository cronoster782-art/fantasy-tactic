...
import type { FormationName } from "@/rival-data";
...
function FichaEnfrentamiento({ jugador }: { jugador: LivePlayer }) {
  ...
  return (
    <div className={styles.fichaEnfrentamiento}>
      ...
      />
      <div
        className={`${styles.puntuacionOverlayCampo} ${getPuntuacionClass(
          jugador.liveScore
        )}`}
      >
        {jugador.liveScore ?? "?"}
...
function ZonaDeJuego({
  ...
  titulo: string;
  jugadoresLocal: LivePlayer[];
  jugadoresVisitante: LivePlayer[];
  resultado: ZonaResultado;
  ventaja?: number;
  puntosLocal: number;
  ...
  const getResultadoClase = (res: ZonaResultado) => {
    switch (res) {
      case "ganando":
        return styles.localGana;
      case "perdiendo":
        return styles.visitanteGana;
      default:
        return styles.empate;
    }
  };
  ...
  const getPointsClass = (teamType: "local" | "visitante") => {
    ...
    if (resultado === "ganando")
      return teamType === "local"
        ? styles.puntosGanando
        : styles.puntosPerdiendo;
    return teamType === "local"
      ? styles.puntosPerdiendo
      : styles.puntosGanando;
  };
  ...
  return (
    <div
      className={`${styles.zonaEnfrentamiento} ${resultadoClase} ${backgroundClass}`}
    >
      <div className={styles.equipoZona}>
        {jugadoresLocal.map((j) => (
          <FichaEnfrentamiento key={j.id} jugador={j} />
        ))}
      </div>
      <div className={styles.marcadorZona}>
        <span className={`${styles.puntosZona} ${getPointsClass("local")}`}>
          {puntosLocal}
        </span>
        <div className={styles.marcadorCentro}>
          <h3>{titulo}</h3>
          <span className={styles.vs}>VS</span>
          {typeof ventaja === "number" && (
            <span className={styles.ventajaLocal}>Local +{ventaja}</span>
          )}
        </div>
        <span className={`${styles.puntosZona} ${getPointsClass("visitante")}`}>
          {puntosVisitante}
        </span>
      </div>
      <div className={styles.equipoZona}>
...
export default function EnfrentamientoPage() {
  ...
  const matchId = idParam ? parseInt(idParam) : NaN;
  ...
  const [localFormation, setLocalFormation] =
    useState<FormationName>("4-4-2");
  const [visitorFormation, setVisitorFormation] =
    useState<FormationName>("4-4-2");
  ...
  useEffect(() => {
    ...
    const localWithScores: LivePlayer[] = rosterLocal.lineup
      .filter((p): p is Jugador => p !== null && p.id != null)
      .map((p) => ({
        ...p,
        id: p.id,
        nombre: p.nombre ?? "Sin nombre",
        liveScore: Math.floor(Math.random() * 17) - 2,
      }));

    const rivalWithScores: LivePlayer[] = rosterVisitante.lineup
      .filter((p): p is Jugador => p !== null && p.id != null)
      .map((p) => ({
        ...p,
        id: p.id,
  ...
  const calculateZoneScore = (
    ...
    players
      .filter((p) => p && p.posicion && positions.includes(p.posicion))
      .reduce((sum, p) => sum + (p.liveScore || 0), 0);
  ...
  const puntos = {
    local: [localDef, localMid, localFwd],
    visitante: [visitanteFwd, visitanteMid, visitanteDef],
  };
  const ventajas = [2, 4, 4];
  const resultados: ZonaResultado[] = [
    puntos.visitante[0] > puntos.local[0] + ventajas[0]
      ? "perdiendo"
      : puntos.local[0] > puntos.visitante[0]
      ? "ganando"
      : "empatando",
    puntos.visitante[1] > puntos.local[1] + ventajas[1]
      ? "perdiendo"
      : puntos.local[1] > puntos.visitante[1]
      ? "ganando"
      : "empatando",
    puntos.visitante[2] > puntos.local[2] + ventajas[2]
      ? "perdiendo"
      : puntos.local[2] > puntos.visitante[2]
      ? "ganando"
      : "empatando",
  ];
      ...
      <div className={styles.card}>
        <div className={styles.header}>
          <div className={styles.headerTeam}>
            <h1>{match.home}</h1>
            <span className={styles.formationDisplay}>
              ({localFormation})
            </span>
          </div>
          <span className={styles.vs}>VS</span>
        ...
        <div className={styles.enfrentamientoContainer}>
          <ZonaDeJuego
            titulo="Ataque vs Defensa"
            jugadoresLocal={equipoLocal.filter(
              (p) => p && p.posicion === "DEL"
            )}
            jugadoresVisitante={equipoVisitante.filter(
              (p) => p && (p.posicion === "DEF" || p.posicion === "POR")
            )}
            resultado={resultados[2]}
            ventaja={4}
            puntosLocal={puntos.local[2]}
            puntosVisitante={puntos.visitante[2]}
            backgroundClass={styles.zonaAtaque}
          />
          <ZonaDeJuego
            titulo="Mediocampo vs Mediocampo"
            jugadoresLocal={equipoLocal.filter(
              (p) => p && p.posicion === "MED"
            )}
            jugadoresVisitante={equipoVisitante.filter(
            ...
            ventaja={4}
            puntosLocal={puntos.local[1]}
            puntosVisitante={puntos.visitante[1]}
            backgroundClass={styles.zonaMedio}
          />
          <ZonaDeJuego
            titulo="Defensa vs Ataque"
            jugadoresLocal={equipoLocal.filter(
              (p) => p && (p.posicion === "POR" || p.posicion === "DEF")
            )}
            jugadoresVisitante={equipoVisitante.filter(
              (p) => p && p.posicion === "DEL"
            )}
            ...
            ventaja={2}
            puntosLocal={puntos.local[0]}
            puntosVisitante={puntos.visitante[0]}
            backgroundClass={styles.zonaDefensa}
          />
        </div>
      </div>
    </main>
